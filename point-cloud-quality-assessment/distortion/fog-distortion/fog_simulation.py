...
def get_available_alphas(INTEGRAL_PATH) -> List[float]:
...
def get_integral_dict(p: ParameterSet, INTEGRAL_PATH) -> Dict:

    alphas = get_available_alphas(INTEGRAL_PATH)
...
def P_R_fog_soft(p: ParameterSet, pc: np.ndarray, original_intesity: np.ndarray, noise: int, gain: bool = False,
                 noise_variant: str = 'v1', INTEGRAL_PATH="") -> Tuple[np.ndarray, np.ndarray, Dict]:
    ...
    integral_dict = get_integral_dict(p, INTEGRAL_PATH)
...
def simulate_fog(p: ParameterSet, pc: np.ndarray, noise: int, gain: bool = False, noise_variant: str = 'v1',
                 hard: bool = True, soft: bool = True, INTEGRAL_PATH="") -> Tuple[np.ndarray, np.ndarray, Dict]:
    ...
    if soft:
        augmented_pc, simulated_fog_pc, num_fog, info_dict = P_R_fog_soft(p, augmented_pc, original_intensity, noise, gain,
                                                                 noise_variant, INTEGRAL_PATH=INTEGRAL_PATH)
...
def parse_arguments():
    ...
    parser.add_argument('-r', '--root_folder', help='Path to the root folder containing the undistorted point clouds.', type=str,
                        default='./data_root/Kitti/')
    parser.add_argument('-pc', '--pc_name', help='Name of the source point cloud to be distorted.', type=str,
                        default='./data_root/Kitti/')
    ...
    parser.add_argument('-a', '--alphas',
                        help='List of attenuation coefficient values. Ensure that each of the chosen values has a precomputed integral.',
                        required=True,
                        type=float,
                        nargs='+')
    parser.add_argument('-p', '--parameters', help='Name of the parameters file containing metadata of the distorted PC',
...
def _map(args):
    i, alphas, lidar_save_path, path, n_features, INTEGRAL_PATH = args
    parameter_set = ParameterSet(alpha=alphas[i])
    points = np.fromfile(path, dtype=np.float32)
    points = points.reshape((-1, n_features))
    points, _, num_fog, _ = simulate_fog(parameter_set, points, 10, gain=True, INTEGRAL_PATH=INTEGRAL_PATH)
    pc_save_path = lidar_save_path / f"{alphas[i]}.bin"
    points.astype(np.float32).tofile(pc_save_path)

if __name__ == '__main__':
    args = parse_arguments()
    INTEGRAL_PATH = Path(os.path.dirname(os.path.realpath(__file__))) / args.inte_folder
    ...
    src_folder = Path(args.root_folder)
    alphas = args.alphas
    path = src_folder / args.pc_name
    print("path", path)
    #all_files = list(src_folder.glob("*.bin"))
    ...
    #parameters_path = Path(dst_folder) / args.parameters
    # parameters = ParameterSet(alpha=args.alpha)
    # parsed_parameters = {key: float(value) for key, value in parameters.__dict__.items()}
    # with open(parameters_path, "w") as f:
    #     yaml.dump(parsed_parameters, f)


    lidar_save_path = Path(dst_folder) / args.pc_name
    lidar_save_path.mkdir(parents=True, exist_ok=True)

    n = len(alphas)
    # Prepare argument tuples for starmap
    map_args = [(i, alphas, lidar_save_path, path, args.n_features, INTEGRAL_PATH) for i in range(n)]
    with mp.Pool(args.n_cpus) as pool:
        l = list(tqdm(pool.map(_map, map_args), total=n))
